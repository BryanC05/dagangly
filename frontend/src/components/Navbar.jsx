...
import { MapPin, ShoppingBag, Store, User, LogOut, Sun, Moon, Globe, MessageCircle, Accessibility, Menu, X } from 'lucide-react';
...
function Navbar() {
  ...
  // Shared nav links content (used in both desktop and mobile)
  const NavLinks = ({ onClick }) => (
    <>
      <Link to="/" className="nav-link" role="menuitem" onClick={onClick}>{t('nav.home')}</Link>
      <Link to="/products" className="nav-link" role="menuitem" onClick={onClick}>{t('nav.products')}</Link>
      <Link to="/nearby" className="nav-link" role="menuitem" onClick={onClick}>
        <MapPin size={18} aria-hidden="true" />
        {t('nav.nearby')}
      </Link>
      <Link to="/cart" className="nav-link cart-link" role="menuitem" onClick={onClick}>
      ...
      <Link to="/forum" className="nav-link" role="menuitem" onClick={onClick}>
  ...
  const NavActions = ({ onClick }) => (
    <>
      ...
      {user ? (
        <>
          {user.role === 'seller' && (
            <Link to="/seller/dashboard" className="btn-seller" role="menuitem" onClick={onClick}>
              <Store size={18} aria-hidden="true" />
              {t('nav.dashboard')}
            </Link>
          )}
          <Link to="/orders" className="nav-link" role="menuitem" onClick={onClick}>
            <ShoppingBag size={18} aria-hidden="true" />
            {t('nav.myOrders')}
          </Link>
          <Link to="/chat" className="nav-link" role="menuitem" onClick={onClick}>
            <MessageCircle size={18} aria-hidden="true" />
            Messages
          </Link>
          <NotificationBell />
          <Link to="/profile" className="nav-link" role="menuitem" onClick={onClick}>
        ...
        <>
          <Link to="/login" className="btn-secondary" onClick={onClick}>{t('nav.login')}</Link>
          <Link to="/register" className="btn-primary" onClick={onClick}>{t('nav.register')}</Link>
        </>
      )}
    </>
  );
        ...
        <div className={`mobile-drawer ${mobileMenuOpen ? 'open' : ''}`}>
          <div className="mobile-drawer-header">
            <img
              ...
              className="logo-image"
            />
            <button onClick={closeMobile} className="mobile-close-btn" aria-label="Close menu">
              <X size={22} />
            </button>
          </div>
          ...
          <div className="mobile-drawer-actions">
            <NavActions onClick={closeMobile} />
          </div>
        </div>
    </>
  );
